...
public class GAMV3 extends ModelBuilderSchema<GAM, GAMV3, GAMV3.GAMParametersV3> {
  public static final class GAMParametersV3 extends ModelParametersSchemaV3<GAMModel.GAMParameters, GAMParametersV3> {
    ...
    @API(help = "Location of knots for each column specified in gam_X", required = true, level = Level.critical, gridable = true)
    public double[][] knots;

    @API(help = "Smoothing parameter for gam predictors", required = false, level = Level.critical, gridable = true)
    public double[] scale;

    @API(help = "Basis function type for each gam predictors", values = {"cr"}, required = false, level = Level.critical, gridable = true)
    public BSType[] bs;
